                ...
                @if ($category->children->isNotEmpty())
                    ...
                    <ul class="dropdown-navigation-submenu absolute hidden group-hover:block bg-white shadow-lg">
                        @foreach ($category->children as $children)
                            <li>
                                <a class="block py-2 px-4 @if ($selectedCategory($children)) text-orange @else text-black @endif hover:text-orange hover:bg-gray-100" href="{{ route('catalog', ['category' => $children]) }}">{{ $children->name }}</a>
                            </li>
                        @endforeach
                    </ul>
                @else
                    <a class="inline-block p-4 @if ($selectedCategory($category)) text-orange @else text-black @endif font-bold hover:text-orange" href="{{ route('catalog', compact('category')) }}">{{ $category->name }}</a>
                @endif
            </li>
        @endforeach()
    </ul>
